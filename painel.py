# -*- coding: utf-8 -*-
"""
Painel de Inteligência Tática - v17.6: Adicionado Template D3 (Pós Rodada)
"""
...
def save_dossier(repo, file_name_template: str, path_parts: list, content: str, required_fields: dict):
    ...
    format_dict = {k: v.replace(' ', '_').replace('/', '-') for k, v in required_fields.items() if k in ['liga', 'pais', 'clube', 'temporada', 'time_casa', 'time_visitante', 'rodada']}
...
if selected_action == "Leitor de Dossiês":
    ...
    if repo:
        ...
        with col2:
            ...
            if st.session_state.get("viewing_file_content"):
                ...
                elif file_name.startswith("D3_"):
                    theme_class = "theme-d3"
    ...
    # --- NOVO TEMPLATE ADICIONADO AQUI ---
        ...
        with st.form("d3_form", clear_on_submit=True):
            ...
            conteudo = st.text_area("Resumo (Conteúdo da Análise)*", height=300, help=help_text_md)
            if st.form_submit_button("Salvar Dossiê", type="primary"):
                # A pasta de País não é pedida, mas é necessária para a hierarquia. Usaremos um placeholder ou podemos adicionar o campo.
                # Por consistência, vamos assumir que a análise pertence a um país.
                pais = st.text_input("País*") # Adicionando o campo que faltava
                if pais:
                    save_dossier(
                        repo, 
                        "D3_Pos_Rodada_{time_casa}_vs_{time_visitante}_R{rodada}", 
                        [pais, liga, temporada, f"Rodada_{rodada}"], 
                        conteudo, 
                        {"time_casa": time_casa, "time_visitante": time_visitante, "rodada": rodada, "conteudo": conteudo}
                    )
                else:
                    st.error("O campo 'País' é necessário para organizar o arquivo.")
