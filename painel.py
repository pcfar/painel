# -*- coding: utf-8 -*-
"""
Painel de Inteligência Tática - v15.3: Correção de NameError e Estabilidade
"""
...
if selected_action == "Leitor de Dossiês":
    ...
    # Função auxiliar para obter valores do st.session_state no modo de edição
    def get_value(key, default=''):
        return st.session_state.get(key, default) if is_edit_mode else default

    if dossier_type == "D1 P1 - Análise da Liga":
        st.subheader("Template: Análise da Liga (Genérico)")
        with st.form("d1_p1_form", clear_on_submit=not is_edit_mode):
            st.warning("Este template usa um renderizador genérico. Para um visual mais rico, use os templates específicos.")
            # ... (código do formulário D1P1) ...
        ...
        with st.form("dominance_form"):
            st.write("**Informações Gerais**"); c1, c2, c3 = st.columns(3); pais = c1.text_input("País*", value=get_value('pais_d1p2')); liga = c2.text_input("Liga*", value=get_value('liga_d1p2')); temporada = c3.text_input("Temporada*", value=get_value('temporada_d1p2'))
            st.divider(); st.write("**Placar de Dominância**"); tabela_raw = st.text_area("Dados da Tabela*", value=get_value('tabela_raw_d1p2'), help="Um time,pontuação por linha. Ex: FC Copenhagen,50")
            st.divider(); st.write("**Análise do Estratega**"); analise_poder = st.text_area("Análise da Estrutura de Poder*", value=get_value('analise_poder_d1p2')); c1, c2 = st.columns(2); indice_top3 = c1.number_input("Índice de Concentração no Top 3 (%)*", min_value=0, max_value=100, value=get_value('indice_top3_d1p2')); comentario_concentracao = c2.text_area("Comentário sobre a Concentração*", value=get_value('comentario_concentracao_d1p2')); evolucao = st.text_area("Análise da Evolução da Competitividade*", value=get_value('evolucao_d1p2'))
            st.divider(); st.write("**Veredito Final** (Top 3)"); c1, c2 = st.columns(2); veredicto1_equipe = c1.text_input("🥇 1º Lugar (Equipe)*", value=get_value('veredicto1_equipe_d1p2')); veredicto1_comentario = c2.text_area("Comentário sobre o 1º Lugar*", value=get_value('veredicto1_comentario_d1p2')); c1, c2 = st.columns(2); veredicto2_equipe = c1.text_input("🥈 2º Lugar (Equipe)*", value=get_value('veredicto2_equipe_d1p2')); veredicto2_comentario = c2.text_area("Comentário sobre o 2º Lugar*", value=get_value('veredicto2_comentario_d1p2')); c1, c2 = st.columns(2); veredicto3_equipe = c1.text_input("🥉 3º Lugar (Equipe)*", value=get_value('veredicto3_equipe_d1p2')); veredicto3_comentario = c2.text_area("Comentário sobre o 3º Lugar*", value=get_value('veredicto3_comentario_d1p2'))
