# -*- coding: utf-8 -*-
"""
Painel de Inteligência Tática - v17.5: Motor de Estilos Dinâmico e Definitivo
"""
...
# --- 1. CONFIGURAÇÃO E PALETAS DE CORES ---
st.set_page_config(page_title="Sistema de Inteligência Tática", page_icon="⚽", layout="wide")

# Paleta de cores centralizada para os temas dos dossiês
THEMES = {
    "default": {"h2": "#38BDF8", "h3": "#FACC15", "strong": "#FACC15", "border": "#3182CE", "marker": "▪", "marker_color": "#63B3ED"},
    "D1P1": {"h2": "#38BDF8", "h3": "#FACC15", "strong": "#FACC15", "border": "#3182CE", "marker": "▪", "marker_color": "#63B3ED"},
    "D1P2": {"h2": "#38BDF8", "h3": "#FACC15", "strong": "#FACC15", "border": "#3182CE", "marker": "▪", "marker_color": "#63B3ED"},
    "D2P1": {"h2": "#34D399", "h3": "#A3E4D3", "strong": "#A3E4D3", "border": "#10B981", "marker": "›", "marker_color": "#34D399"},
    "D2P2": {"h2": "#F87171", "h3": "#F97316", "strong": "#F97316", "border": "#DC2626", "marker": "»", "marker_color": "#F87171"},
}

def apply_custom_styling(theme_colors):
    """Aplica o CSS estrutural e injeta as cores do tema dinâmico."""
    st.markdown(f"""
        <style>
            ...
            /* ESTRUTURA BASE DOS TÍTULOS (sem cor) */
            ...
            .dossier-viewer h1 {{ border-bottom: 3px solid {theme_colors['border']}; }}
            .dossier-viewer h2 {{ color: {theme_colors['h2']}; border-left: 4px solid {theme_colors['h2']}; }}
            .dossier-viewer h3, .dossier-viewer strong {{ color: {theme_colors['h3']}; }}
            .dossier-viewer li::before {{ content: "{theme_colors['marker']}"; color: {theme_colors['marker_color']}; margin-right: 12px; font-size: 1.2rem; }}
            .dossier-viewer table th {{ background-color: {theme_colors['border']}; }}
        </style>
    """, unsafe_allow_html=True)

# --- 2. FUNÇÕES AUXILIARES ---
def sanitize_text(text: str) -> str: return text.replace('\u00A0', ' ').replace('\u2011', '-')
@st.cache_resource
def get_github_repo():
...
def save_dossier(repo, file_name_template: str, path_parts: list, content: str, required_fields: dict):
    ...
    file_name = file_name_template.format(**format_dict) + ".md"; final_path_parts = [p.replace(" ", "_") for p in path_parts]; full_path = "/".join(final_path_parts) + "/" + file_name
...
repo = get_github_repo() # Movido para cima para estar disponível para todas as abas

# Define o tema padrão antes de qualquer outra coisa
theme_colors = THEMES.get("default")
if st.session_state.get("viewing_file_name"):
    file_name = st.session_state.get("viewing_file_name")
    prefix = file_name.split('_')[0]
    theme_colors = THEMES.get(prefix, THEMES["default"])
apply_custom_styling(theme_colors)
