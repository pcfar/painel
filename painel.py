# -*- coding: utf-8 -*-
"""
Painel de Inteligência Tática - v4.1: Nomenclatura Dinâmica para Dossiês de Liga
"""
...
def apply_custom_styling():
    ...
    # O CSS permanece o mesmo da versão anterior, pois já é robusto.
    st.markdown("""
        <style>
...
def parse_path_to_form(path):
    """Analisa o caminho do arquivo para preencher o formulário, agora com suporte a nomes de arquivo dinâmicos."""
    try:
        ...
        # --- MUDANÇA AQUI: Lógica de reconhecimento de tipo de dossiê melhorada ---
        if file_name.startswith("Dossie_") and len(parts) == 4:
            ...
            with st.form("dossier_form", clear_on_submit=False):
                ...
                if st.form_submit_button(submit_label, type="primary", use_container_width=True):
                    if not all([st.session_state.pais, st.session_state.liga, st.session_state.temporada, st.session_state.conteudo_md]): st.error("Todos os campos com * são obrigatórios.")
                        ...
                        # --- MUDANÇA AQUI: Lógica de criação de nome de arquivo para Liga ---
                        if tipo == "Dossiê de Liga":
                            liga_formatada = st.session_state.liga.replace(' ', '_')
                            file_name = f"Dossie_{liga_formatada}.md"
