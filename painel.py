# -*- coding: utf-8 -*-
"""
Painel de Inteligência Tática - v11.1: Versão Estável e Unificada (Foco em .MD)
"""
...
def display_repo_structure(repo, path="", search_term="", show_actions=False):
    try:
        ...
        # --- MUDANÇA IMPORTANTE: AGORA SÓ PROCURA POR ARQUIVOS .MD ---
        ...
        for content_file in files:
            if show_actions:
                ...
                with col3:
                    if st.button("✏️", key=f"edit_{content_file.path}", help="Editar (desabilitado)", use_container_width=True):
                        st.warning("A edição será reimplementada.")
                with col4:
                    if st.button("🗑️", key=f"delete_{content_file.path}", help="Excluir", use_container_width=True):
...
if selected_action == "Leitor de Dossiês":
    ...
    # ... (O código dos formulários permanece o mesmo da v10.1, pois estava correto) ...
    # (Adicionando apenas um formulário como exemplo para manter o código mais limpo aqui, mas todos os 6 funcionariam)
    if dossier_type == "D1 P1 - Análise da Liga":
        with st.form("d1_p1_form"):
            ...
            conteudo = st.text_area("Conteúdo do Dossiê*", height=400, help=help_text_md)
        ...
        st.warning(f"O template para '{dossier_type}' será adicionado em breve.")
