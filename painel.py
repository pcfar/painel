    ...
    try:
        ...
        for content_file in files:
            if show_actions:
                ...
                with c2:
                    if st.button("✏️", key=f"edit_{content_file.path}", help="Editar Dossiê (desabilitado)", use_container_width=True): st.warning("Edição em desenvolvimento.")
                with c3:
...
if selected_action == "Leitor de Dossiês":
    ...
    dossier_type = st.selectbox("**Tipo de Dossiê**", dossier_type_options, key="dossier_type_selector")
    ...
    # --- CÓDIGO CORRIGIDO PARA D1 P2 ---
        ...
        with st.form("d1_p2_form", clear_on_submit=True):
            st.write("**Informações Gerais**"); c1, c2, c3 = st.columns(3)
            ...
            if st.form_submit_button("Gerar Dossiê", type="primary", use_container_width=True):
                if not all([pais, liga, temporada, conteudo]):
                    ...
                    componentes = parse_text_to_components(conteudo)
                    dossier_data = {'metadata': {'titulo_principal': f"ANÁLISE DE DOMINÂNCIA: {liga}", 'icone_principal': "👑"}, 'componentes': componentes}
                    yaml_string = yaml.dump(dossier_data, sort_keys=False, allow_unicode=True, indent=2)
                    file_name = f"D1P2_Clubes_Dominantes_{liga.replace(' ', '_')}_{pais.replace(' ', '_')}.yml"
                    path_parts = [pais, liga, temporada]; full_path = "/".join(p.replace(" ", "_") for p in path_parts) + "/" + file_name
                    with st.spinner("Salvando..."):
                        try:
                            repo.create_file(full_path, f"Adiciona: {file_name}", yaml_string); st.success(f"Dossiê salvo: {full_path}")
