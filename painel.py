# -*- coding: utf-8 -*-
"""
Painel de Inteligência Tática - v14.4: Template Customizado para Análise de Dominância
"""
...
def apply_custom_styling():
    st.markdown("""
        <style>
            ...
            /* --- ESTILOS PARA O NOVO TEMPLATE DE DOMINÂNCIA (TEMA ESCURO) --- */
            .dominance-box {
...
def render_dominance_dossier(data: dict):
    ...
    tabela_html = ""
    for i, item in enumerate(data.get('tabela_dominancia', [])):
        tabela_html += f"<tr><td>{i+1}</td><td>{item.get('equipe')}</td><td>{item.get('pontos')}</td></tr>"
...
def display_repo_structure(repo, path="", search_term="", show_actions=False):
    # (Esta função permanece a mesma da v12.1)
    pass # Omitido para brevidade, o código completo está abaixo
...
# Copiando a função display_repo_structure completa para garantir
...
if selected_action == "Leitor de Dossiês":
    ...
    if dossier_type == "D1 P2 - Análise dos Clubes Dominantes da Liga":
        ...
        with st.form("dominance_form", clear_on_submit=True):
            ...
            st.divider(); st.write("**Placar de Dominância**"); tabela_raw = st.text_area("Dados da Tabela*", help="Um time e sua pontuação por linha, separados por vírgula. Ex: FC Copenhagen, 50")
            st.divider(); st.write("**Análise do Estratega**"); analise_poder = st.text_area("Análise da Estrutura de Poder*"); c1, c2 = st.columns(2); indice_top3 = c1.number_input("Índice de Concentração no Top 3 (%)*", min_value=0, max_value=100); comentario_concentracao = c2.text_area("Comentário sobre a Concentração*"); evolucao = st.text_area("Análise da Evolução da Competitividade*")
            st.divider(); st.write("**Veredito Final** (Top 3)"); c1, c2 = st.columns(2); veredicto1_equipe = c1.text_input("🥇 1º Lugar (Equipe)*"); veredicto1_comentario = c2.text_area("Comentário sobre o 1º Lugar*"); c1, c2 = st.columns(2); veredicto2_equipe = c1.text_input("🥈 2º Lugar (Equipe)*"); veredicto2_comentario = c2.text_area("Comentário sobre o 2º Lugar*"); c1, c2 = st.columns(2); veredicto3_equipe = c1.text_input("🥉 3º Lugar (Equipe)*"); veredicto3_comentario = c2.text_area("Comentário sobre o 3º Lugar*")
            if st.form_submit_button("Gerar Dossiê de Dominância", type="primary", use_container_width=True):
                ...
                with st.spinner("Salvando..."):
                    try: repo.create_file(full_path, f"Adiciona: {file_name}", yaml_string); st.success(f"Dossiê salvo com sucesso: {full_path}")
